...
#pragma omp parallel for private(lz,ly,lx,v,mu,nu,c1,c2)
		for(v3=0; v3<lv3; v3++) {
			...
			for(c2=0;c2<3;c2++){  
			  //pion
			  udblock[v3] = qcd_CSUB( udblock[v3],qcd_CMUL(dprop_pb.D[v][mu][nu][c1][c2],qcd_CONJ(uprop_pb.D[v][mu][nu][c1][c2])) );
			  dublock[v3] = qcd_CSUB( dublock[v3],qcd_CMUL(uprop_pb.D[v][mu][nu][c1][c2],qcd_CONJ(dprop_pb.D[v][mu][nu][c1][c2])) );
			  //-kaon
